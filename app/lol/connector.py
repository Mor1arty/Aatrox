...
class LolClientConnector(QObject):
    ...
    @retry()
    async def createARAMLobby(self, lobbyName, password):
        data = {
            "customGameLobby": {
                "configuration": {
                    "gameMode": "CLASSIC",
    ...
    @retry()
    async def getLobbyMembers(self):
        res = await self.__get("/lol-lobby/v2/lobby/members")
        return await res.json()
    
    async def switchTeam(self):
        await self.__post('/lol-lobby/v1/lobby/custom/switch-teams')
    
    @retry()
    async def inviteSummonersToLobby(self, summonerIds):
        data = [
            {"toSummonerId" : str(summonerId)} for summonerId in summonerIds
        ]
        await self.__post("/lol-lobby/v2/lobby/invitations", data=data)

    @retry()
    async def startChampionSelect(self):
        await self.__post("/lol-lobby/v1/lobby/custom/start-champ-select")
